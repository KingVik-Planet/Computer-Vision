...
while True:
    ...
    for contour in contours:
        ...
        cv2.rectangle(gray_frame, (x, y), (x + w, y + h), (255, 0, 0), 3)
        ...
        for (fx, fy, fw, fh) in faces:
            ...
            center_y = fy + fh // 2
            radius = max(fw, fh) // 2
            cv2.circle(gray_frame, (center_x, center_y), radius, (0, 255, 0), 2)

        # Start recording if motion detected
        if not recording:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = os.path.join(output_dir, f"Capture_Grayscale_{timestamp}.avi")
            fourcc = cv2.VideoWriter_fourcc(*'XVID')
            # Set grayscale (single-channel) frame size for recording
            out = cv2.VideoWriter(output_path, fourcc, 20.0, (gray_frame.shape[1], gray_frame.shape[0]), isColor=False)
            recording = True

        status = 1
        # Adding timestamp
        timestamp = datetime.now().strftime("Cam1: Date:%d/%m/%Y:Time:%H:%M:%S")
        cv2.putText(gray_frame, timestamp, (10, 20), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 0), 1)

    # Write the grayscale frame if recording
    if recording:
    ...
    cv2.imshow("Grayscale Screen", gray_frame)
